...
class_name LED_yellow
var led_sprite: Sprite2D
var light: PointLight2D
var texture_on = preload("res://graphics/legacy/led/ld_up_yellow.png")
var texture_off = preload("res://graphics/legacy/led/ld_down_yellow.png")
...
func initialize(spec: ComponentSpecification, ic = null)->void:
	...
	if(GlobalSettings.CurrentGraphicsMode==DefaultGraphicsMode):
		...
		led_sprite.modulate = Color(100, 100, 0, 0.2)
	else:
		led_sprite.texture = texture_off
	readable_name = "Светодиод (жел.)"
...
func set_on():
	if GlobalSettings.CurrentGraphicsMode==LegacyGraphicsMode:
		...
		sprite.modulate = Color(100, 100, 0, 1)
	light.enabled = true
