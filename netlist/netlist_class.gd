...
func propagate_signal() -> void:
	if GlobalSettings.doCycles:
		...
		for pin in nodes.keys():
			if pin.output():
				resolved.append(pin)
				for neighbour in nodes[pin].neighbours:
					if neighbour.pin.output():
						if neighbour.pin in resolved and neighbour.pin.state != pin.state:
							if not neighbour.pin.z and not pin.z:
								PopupManager.display_error("Соединены два выхода", "Вы делаете что-то странное", pin.global_position)
			else:
				var lateprop = true
				for neighbour in nodes[pin].neighbours:
					if neighbour.pin in resolved:
						lateprop = false
				if lateprop:
					late_propagation.append(pin)
					continue
				var state = nodes[pin].neighbours[0].pin.state
				var ok = true
				for neighbour in nodes[pin].neighbours:
					if neighbour.pin not in resolved:
						continue
